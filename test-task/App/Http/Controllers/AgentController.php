...
class AgentController extends Controller
{
    ...
    {
        // Define API secret key
        $apiSecret = env('API_SECRET_KEY');  // Store this in your .env file
        Log::info("Received API SECRET KEY is ",[
            "Key" => $apiSecret
        ]);
        ...
        $signString = hash('sha256', $dataString . $apiSecret);  // Appendiing secret and hash
    ...
    public function modifyPassword(Request $request)
    {
        ...
        ksort($data); // Sort by keys in ascending order
        $dataString = implode('', $data); // Concatenate values
        $generatedSign = hash('sha256', $dataString . $apiSecret); // Append secret and hash
        ...
        // Check if the agent account exists
        $agent = AgentAccount::on('mysql_master')->where('agent', $account)->first();
        if (!$agent) {
